...
| 选项 | 说明 | 多种支付方式 | 单一支付方式 |
|------|------|-------------|-------------|
| [托管支付页面集成](#托管支付页面集成) | 启动完整的、由 SDK 管理的支付流程，提供预构建的支付方式选择、卡信息输入和结账页面。支持自定义主题色和深色模式。**推荐用于大多数场景。** | <img src="assets/hosted_payment_page.png" width="200" alt="托管支付页面 - 多种支付方式"> | <img src="assets/hosted_payment_page_2.png" width="200" alt="托管支付页面 - 单一支付方式"> |
| [嵌入式元素集成](#嵌入式元素集成) | 通过 Jetpack Compose 将 Airwallex 的 `PaymentElement` 直接嵌入到你的 Activity 或 View 中。你可以完全控制宿主布局和导航，同时利用 SDK 的支付 UI 组件。 | <img src="assets/embedded_element.png" width="200" alt="嵌入式元素 - 多种支付方式"> | <img src="assets/embedded_element_2.png" width="200" alt="嵌入式元素 - 单一支付方式"> |
...
在 Application 类中配置 SDK（与托管支付页面集成相同 - 参见 [SDK 配置](#sdk-配置)）。
