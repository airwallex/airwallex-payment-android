...
| [locale](locale.md) | [androidJvm]<br>open val [locale](locale.md): [Locale](https://developer.android.com/reference/kotlin/java/util/Locale.html)?<br>Locale used by Airwallex-owned UI. `null` means the SDK inherits the host application's current locale. |
