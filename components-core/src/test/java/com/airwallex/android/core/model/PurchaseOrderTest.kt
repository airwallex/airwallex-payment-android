...
class PurchaseOrderTest {
    ...
    fun builderConstructor() {
        assertEquals(order, PurchaseOrderFixtures.ORDER)
    }

    @Test
    fun testParcelable() {
        assertEquals(PurchaseOrderFixtures.ORDER, ParcelUtils.create(PurchaseOrderFixtures.ORDER))
    }
    ...
    @Test
    fun testToParamsMap() {
        val paramMap = order.toParamMap()
        assertEquals(
            mapOf(
                "products" to products.map { it.toParamMap() },
                "shipping" to shipping.toParamMap(),
                "type" to "physical_goods"
            ),
            paramMap
        )
    }
