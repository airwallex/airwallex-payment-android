...
dependencies {
    ...
    implementation 'io.github.airwallex:payment-card:6.7.0'        // Card payments
    implementation 'io.github.airwallex:payment-redirect:6.7.0'    // Redirect-based payments
    implementation 'io.github.airwallex:payment-wechat:6.7.0'      // WeChat Pay
    implementation 'io.github.airwallex:payment-googlepay:6.7.0'   // Google Pay
}
```
...
```groovy
    dependencies {
       ...
       implementation 'io.github.airwallex:payment-card:6.7.0'//only support card
       implementation 'io.github.airwallex:payment-googlepay:6.7.0'//only support google pay
       implementation 'io.github.airwallex:payment-redirect:6.7.0'//only support redirect
    }
```
...
```
> **⚠️ Deprecated:** `airwallex.confirmPaymentIntent(...)` is deprecated. Use `airwallex.checkout(...)` for all card and consent-based payments, and `airwallex.startGooglePay(...)` for Google Pay.
...
airwallex.checkout(
    ...
    cvc = "737",
    saveCard = false, // set to true to save the card as a consent while making the payment
    listener = object : Airwallex.PaymentResultListener {
        override fun onCompleted(status: AirwallexPaymentStatus) {
            // You can handle different payment statuses and perform UI action respectively here
        }
