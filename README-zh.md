...
- [低层 API 集成](#低层-api-集成)
  ...
  - [可用 API](#可用-api)
      ...
      - [重定向支付](#重定向如支付宝hk)
      - [用卡和账单详情进行 Checkout](#用卡和账单详情进行-checkout)
      - [用 Consent ID 进行 Checkout](#用-consent-id-进行-checkout)
      - [用 PaymentConsent 进行 Checkout](#用-paymentconsent-进行-checkout)
      - [获取支付方式列表](#获取支付方式列表)
      - [获取已保存卡列表](#获取已保存卡列表)
- [迁移指南](./MIGRATING.md)
- [贡献与反馈](#贡献与反馈)
...
从旧版本升级？请参阅[迁移指南](./MIGRATING.md)。

### 支持的支付方式

| 类别 | 支付方式 | 备注 |
|------|---------|------|
| 银行卡 | Visa、Mastercard、UnionPay、Discover、JCB、Diners Club | 使用[低层 API 集成](#低层-api-集成)时需 PCI-DSS 合规 |
| Google Pay | [Google Pay](#google-pay-集成) | 需在 Airwallex 账号开通 Google Pay |
| 电子钱包 | 支付宝、支付宝HK、DANA、GCash、Kakao Pay、Touch 'n Go、微信支付，及[更多](https://www.airwallex.com/docs/payments/payment-methods/payment-methods-overview) | 跳转类钱包通常需要 `payment-redirect` 和/或 `payment-wechat` 模块 |
...
默认情况下，Airwallex UI 使用宿主 Android Activity 的 Locale。如需只为某个支付
Session 指定 Locale，并且不修改宿主应用的全局 Locale，可以进行以下配置：
...
该 Locale 会应用于 Airwallex 自有的原生 UI。更改 Locale 时需要创建新的 Session。Google Pay、外部浏览器、支付应用和发卡行
...
Airwallex Android SDK 提供预构建 UI 组件，用于在 Android 应用中快速接入支付。
...
你可以自定义主题色和深色模式，同时适用于托管支付页面和嵌入式元素。

通过 Android 资源覆盖默认主题色：

```xml
<color name="airwallex_tint_color">@color/your_custom_color</color>
```

也可以在 `PaymentElementConfiguration` 中设置 `Appearance.themeColor` 和 `Appearance.isDarkTheme`（参见[配置选项](#配置选项)）。
...
两种集成方式都支持通过 `PaymentElementConfiguration` 进行相同的自定义选项（主题色、深色模式、按钮文本、Google Pay 显示）。

**关键区别 - Session 创建：**
- **托管支付页面（HPP）**：在调用方 Activity 中创建 session → 传递给 `AirwallexStarter` 方法
- **嵌入式元素**：在托管 `PaymentElement` 的 Activity（或其 ViewModel）中创建 session → 传递给 `PaymentElement.create()`
...
在 Application 类中配置 SDK（与托管支付页面相同 — 参见[初始化](#初始化)）。
...
**注意：** 默认情况下，`supportedCardBrands` 包含 `AirwallexSupportedCard` 中的所有卡（Visa、Amex、Mastercard、Discover、JCB、Diners Club、UnionPay）。所有其他参数都是可选的，具有合理的默认值。
...
val configuration = PaymentElementConfiguration.PaymentSheet(
    ...
    googlePayButton = PaymentElementConfiguration.GooglePayButton(
        showsAsPrimaryButton = true,         // true: 显示为主按钮，false: 在列表中显示
        buttonType = null                    // null: 自动（一次性支付为 BUY，循环支付为 SUBSCRIBE）
    ),
...
**Google Pay 按钮：**
- `showsAsPrimaryButton = true` - Google Pay 显示为其他支付方式上方的突出按钮
- `showsAsPrimaryButton = false` - Google Pay 与其他支付方式一起显示在列表中
- `buttonType` - 按钮类型（BUY、SUBSCRIBE 等），null 为自动选择

**结账按钮：**
- `title` - 自定义按钮文本，null 为自动（"支付"或"确认"）

**支付外观：**
- `themeColor` - 自定义主题色，ARGB 格式（如 `0xFF612FFF`），null 为默认值
- `isDarkTheme` - true（深色模式）、false（浅色模式）、null（跟随系统）
...
class CheckoutActivity : ComponentActivity() {
    ...
    private fun setupPaymentElement() {
        ...
        lifecycleScope.launch {
            // 在此 activity 中创建 session（参见"创建 Airwallex Session"部分）
            // 为了更好的架构设计，建议在 ViewModel 中创建并存储 session
            // 以应对配置更改
...
| 功能 | 托管支付页面 | 嵌入式元素 |
|------|--------------|-----------|
| 入口点 | `AirwallexStarter.presentEntirePaymentFlow()`（或 `presentCardPaymentFlow()`） | `PaymentElement.create()` |
| Activity 所有权 | SDK 拥有 activity | 你拥有 activity |
| UI 容器 | SDK 的 activity | 你的 `ComposeView` |
| 布局控制 | 有限（SDK 控制） | 完全（你控制周围 UI） |
| 初始化 | 启动 activity | Suspend 函数（Kotlin）或回调（Java） |
| 结果回调 | `Airwallex.PaymentResultListener` | `PaymentFlowListener` 或 lambda |
...
> **⚠️ 已废弃：** `airwallex.confirmPaymentIntent(...)` 已废弃。所有银行卡和 consent 类支付请使用 `airwallex.checkout(...)`，Google Pay 请使用 `airwallex.startGooglePay(...)`。
...
airwallex.checkout(
    ...
    cvc = "737",
    saveCard = false, // 设为 true 表示在支付的同时将卡片保存为 consent
    listener = object : Airwallex.PaymentResultListener {
        override fun onCompleted(status: AirwallexPaymentStatus) {
            // 处理不同支付状态
        }
...
> **⚠️ 已废弃：** 此流程已废弃，请改用 [用 PaymentConsent 进行 Checkout](#用-paymentconsent-进行-checkout)，并传入至少包含 `id` 和 `nextTriggeredBy` 的 `PaymentConsent`。`airwallex.checkout(...)` 中的 `paymentConsentId` 参数仅为兼容旧版集成而保留，不应在新代码中使用。
